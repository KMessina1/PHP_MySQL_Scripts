...
/* INITIALIZE: TaxJar, Shippo, Stripe, Yaml Libraries */
...
\Stripe\Stripe::setApi($stripe); // STRIPE: Secret 
\Shippo::setApi($shippo); // SHIPPO: API Key
...
function sendErrorEmailToCompany() {
    global $version,$scriptName,$dashes,$timestamp,$stack,$records,$success,$found,$customer,
            $orderID,$errorMsg,$error_CMS,$error_Stripe,$error_Shippo,$error_TaxJar,$errorCount;
    ...
    $calledfromAppName = "SF-App";
    $calledfromAppVersion = "Script v$version";

    $error = "errorMsg: $errorMsg<br>";
    $error .= "error_CMS: $error_CMS<br>";
    $error .= "error_Stripe: $error_Stripe<br>";
    $error .= "error_Shippo: $error_Shippo<br>";
    $error .= "error_TaxJar: $error_TaxJar<br>";
    $error .= "errorCount: $errorCount";

    send_MailerIssueEmail(
        $scriptName,
        $error,
        $orderID,
        $orderDate,
    ...
    writeToStackAndConsole(__LINE__,"PAYSCRIPT:","Send Issue Email.");
